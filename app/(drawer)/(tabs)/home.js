...
// ─── AI Tool Card ─────────────────────────────────────────────────────────────
function AIToolCard({ icon, label, color, badge, onPress, delay, masterAnim }) {
  const scaleAnim = useRef(new Animated.Value(1)).current;
  ...
  const handlePressIn = () => {
    Animated.spring(scaleAnim, { toValue: 0.94, useNativeDriver: true, speed: 30, bounciness: 4 }).start();
  };
  const handlePressOut = () => {
    Animated.spring(scaleAnim, { toValue: 1, useNativeDriver: true, speed: 24, bounciness: 6 }).start();
  };
...
// ─── Template Card ────────────────────────────────────────────────────────────
function TemplateCard({ name, tag, color, onPress }) {
  const scaleAnim = useRef(new Animated.Value(1)).current;
  return (
    <Pressable
    ...
    >
      <Animated.View style={[styles.templateCard, { transform: [{ scale: scaleAnim }] }]}>
        <View style={[styles.templatePreview, { backgroundColor: `${color}18`, borderColor: `${color}30` }]}>
          {/* Template preview lines */}
          <View style={[styles.tLine, styles.tLineHeader, { backgroundColor: color }]} />
          <View style={[styles.tLine, { backgroundColor: `${color}60`, width: "70%" }]} />
          <View style={[styles.tLine, { backgroundColor: `${color}40`, width: "50%" }]} />
          <View style={[styles.tDivider, { backgroundColor: `${color}30` }]} />
          <View style={[styles.tLine, { backgroundColor: `${color}40`, width: "90%" }]} />
          <View style={[styles.tLine, { backgroundColor: `${color}30`, width: "75%" }]} />
          <View style={[styles.tLine, { backgroundColor: `${color}30`, width: "60%" }]} />
        </View>
...
export default function HomeScreen() {
  ...
  // TODO: Replace with real user data from AsyncStorage/context
  const user = { name: "Shanu" };
  const hasResume = false; // Change to true to see returning user state
  ...
  const TEMPLATES = [
    { name: "Modern", tag: "Popular", color: "#4ADE80" },
    { name: "Classic", tag: "Clean", color: "#60A5FA" },
    { name: "Creative", tag: "Bold", color: "#A78BFA" },
  ];
  ...
  return (
    <View style={styles.root}>
      ...
      <View
        pointerEvents="none"
        style={{
          height: Math.max(insets.top, Platform.OS === "android" ? 12 : 0),
        }}
      />
  ...
  />
        ...
        <Animated.View style={animStyle(cardAnim)}>
          <ResumeCard
            ...
            onCreateNew={() => router.push("/(tabs)/builder")}
          />
        </Animated.View>
        ...
        <Animated.View style={animStyle(toolsAnim)}>
          <SectionTitle
            title="AI Tools"
            actionLabel="See all"
            onAction={() => router.push("/(tabs)/analyzer")}
          />
          <ScrollView
            horizontal
