...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function readAchievementsFromStore() {
  return useResumeStore.getState().activeResume.achievements ?? [];
}

function newAchievementItem() {
  return {
...
function AchievementInput({ label, value, onChangeText, placeholder, multiline = false, ...props }) {
...
export default function AchievementsScreen() {
  ...
  const addAchievementToStore = useResumeStore((s) => s.addAchievement);
  const updateAchievementInStore = useResumeStore((s) => s.updateAchievement);
  const removeAchievementFromStore = useResumeStore((s) => s.removeAchievement);
  const markSaved = useResumeStore((s) => s.markSaved);

  // Local State
  const [achievements, setAchievements] = useState(readAchievementsFromStore);
  ...
  useFocusEffect(
    useCallback(() => {
      const storeItems = readAchievementsFromStore();
      setAchievements(storeItems);
  ...
  const handleAddAchievement = () => {
    ...
    const newItem = newAchievementItem();
  ...
  const handleSave = () => {
    ...
    achievements.forEach((item) => {
      if (storeIds.includes(item.id)) {
        // Fallback check if individual update function is missing in store
        const state = useResumeStore.getState();
        if (state.updateAchievement) {
            state.updateAchievement(item.id, item);
        }
      } else {
        if (addAchievementToStore) addAchievementToStore(item);
      }
    });

    const currentIds = achievements.map((item) => item.id);
    readAchievementsFromStore().forEach((item) => {
      if (!currentIds.includes(item.id)) {
        if (removeAchievementFromStore) removeAchievementFromStore(item.id);
      }
    });

    // Manual full array update fallback
    useResumeStore.setState((state) => ({
      activeResume: { ...state.activeResume, achievements: achievements },
      isDirty: true
    }));
          ...
          <Animated.View style={{ opacity: enterOpacity, transform: [{ translateY: enterTranslateY }], gap: theme.spacing.xl }}>
            ...
            <View style={styles.topBar}>
              ...
              <Pressable onPress={() => router.back()} style={styles.skipBtn} hitSlop={12}>
                <Text style={styles.skipText}>{achievements.length > 0 ? "Done" : "Skip"}</Text>
              </Pressable>
            </View>

            {/* ── Subtitle & Badge ── */}
            <View style={styles.sectionHeaderRow}>
              <Text style={styles.screenSub}>Showcase your awards and recognitions</Text>
              {achievements.length > 0 && (
                <View style={styles.countBadge}>
                  <Text style={styles.countBadgeText}>{achievements.length} added</Text>
                </View>
              )}
            </View>
            ...
            {achievements.length === 0 && (
              <View style={styles.emptyWrap}>
                <Ionicons name="trophy-outline" size={36} color={theme.colors.textWhite30} />
                <Text style={styles.emptyText}>No achievements yet</Text>
                <Text style={styles.emptySubtext}>Add awards, scholarships, or special recognitions.</Text>
              </View>
            )}
                  ...
                  <View key={item.id} style={[styles.card, isExpanded && styles.cardExpanded]}>
                    ...
                    {isExpanded && (
                      <View style={styles.cardBody}>
                        ...
                        <View style={styles.row}>
                          <View style={[styles.flex1, { flex: 2 }]}>
                            <AchievementInput 
                              ...
                              placeholder="e.g. Google, Oxford Univ."
                        ...
                        <AchievementInput 
                          label="DESCRIPTION / IMPACT (OPTIONAL)" 
                          placeholder="• Awarded to top 2% of class&#10;• Competed against 500+ participants"
                          multiline
                        ...
                        >
                          ...
                          <Text style={styles.deleteBtnText}>Remove Achievement</Text>
                        </Pressable>
                      </View>
                    )}
                  </View>
            ...
            {/* ── Add Button ── */}
            <Pressable onPress={handleAddAchievement} style={({ pressed }) => [styles.addBtn, pressed && styles.addBtnPressed]}>
              <Ionicons name="add" size={18} color={theme.colors.brandMint} />
              <Text style={styles.addBtnText}>Add Another Achievement</Text>
            </Pressable>
            ...
            {achievements.length > 0 && (
              ...
              >
                <Animated.View style={[styles.saveBtn, { transform: [{ scale: saveScale }] }]}>
                  ...
                  <Text style={styles.saveBtnText}>Save Achievements</Text>
                </Animated.View>
              </Pressable>
            )}
