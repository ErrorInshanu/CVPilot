...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function readPublicationsFromStore() {
  return useResumeStore.getState().activeResume.publications ?? [];
}

function newPublicationItem() {
  return {
...
function PublicationInput({ label, value, onChangeText, placeholder, multiline = false, ...props }) {
...
export default function PublicationsScreen() {
  ...
  const addPublicationToStore = useResumeStore((s) => s.addPublication);
  const updatePublicationInStore = useResumeStore((s) => s.updatePublication);
  const removePublicationFromStore = useResumeStore((s) => s.removePublication);
  const markSaved = useResumeStore((s) => s.markSaved);

  // Local State
  const [publications, setPublications] = useState(readPublicationsFromStore);
  ...
  useFocusEffect(
    useCallback(() => {
      const storeItems = readPublicationsFromStore();
      setPublications(storeItems);
  ...
  const handleAddPublication = () => {
    ...
    const newItem = newPublicationItem();
  ...
  const handleSave = () => {
    ...
    publications.forEach((item) => {
      if (storeIds.includes(item.id)) {
        ...
        if (state.updatePublication) {
            state.updatePublication(item.id, item);
        }
      } else {
        if (addPublicationToStore) addPublicationToStore(item);
      }
    });

    const currentIds = publications.map((item) => item.id);
    readPublicationsFromStore().forEach((item) => {
      if (!currentIds.includes(item.id)) {
        if (removePublicationFromStore) removePublicationFromStore(item.id);
      }
    });

    // Manual full array update fallback
    useResumeStore.setState((state) => ({
      activeResume: { ...state.activeResume, publications: publications },
      isDirty: true
    }));
        ...
        >
          <Animated.View style={{ opacity: enterOpacity, transform: [{ translateY: enterTranslateY }], gap: theme.spacing.xl }}>
            ...
            <View style={styles.topBar}>
              ...
              <Text style={styles.screenTitle}>Publications</Text>
              <Pressable onPress={() => router.back()} style={styles.skipBtn} hitSlop={12}>
                <Text style={styles.skipText}>{publications.length > 0 ? "Done" : "Skip"}</Text>
              </Pressable>
            </View>

            {/* ── Subtitle & Badge ── */}
            <View style={styles.sectionHeaderRow}>
              <Text style={styles.screenSub}>Highlight your research, articles, & books</Text>
              {publications.length > 0 && (
                <View style={styles.countBadge}>
                  <Text style={styles.countBadgeText}>{publications.length} added</Text>
                </View>
              )}
            </View>
            ...
            {publications.length === 0 && (
              <View style={styles.emptyWrap}>
                <Ionicons name="book-outline" size={36} color={theme.colors.textWhite30} />
                <Text style={styles.emptyText}>No publications yet</Text>
                <Text style={styles.emptySubtext}>Add research papers, articles, or conference journals.</Text>
              </View>
            )}
                      ...
                      <View style={styles.cardHeaderLeft}>
                        ...
                        <View style={styles.cardTitleBox}>
                          ...
                          <Text style={styles.cardSubtitle} numberOfLines={1}>
                            {item.publisher ? `${item.publisher} • ` : ""}{item.date || "No date"}
                          </Text>
                        </View>
                      </View>
                      <Ionicons 
                    ...
                    {isExpanded && (
                      <View style={styles.cardBody}>
                        <PublicationInput 
                          ...
                          placeholder="e.g. Deep Learning for Diagnostics"
                          value={item.title}
                        ...
                        <PublicationInput 
                          ...
                          onChangeText={(v) => handleUpdate(item.id, "publisher", v)}
                        />

                        <PublicationInput 
                          label="AUTHORS / CO-AUTHORS" 
                          placeholder="e.g. Shanu, John Doe"
                          value={item.authors}
                          onChangeText={(v) => handleUpdate(item.id, "authors", v)}
                        />
                        ...
                        <View style={styles.row}>
                          ...
                          <View style={[styles.flex1, { flex: 1.5 }]}>
                            <PublicationInput 
                              ...
                              autoCapitalize="none"
                              value={item.url}
                              onChangeText={(v) => handleUpdate(item.id, "url", v)}
                            />
                          </View>
                        </View>
                        ...
                        <PublicationInput 
                          ...
                          placeholder="• Briefly describe the research impact or key findings of this publication."
                          multiline
                        ...
                        >
                          ...
                          <Text style={styles.deleteBtnText}>Remove Publication</Text>
                        </Pressable>
                      </View>
                    )}
                  </View>
            ...
            {/* ── Add Button ── */}
            <Pressable onPress={handleAddPublication} style={({ pressed }) => [styles.addBtn, pressed && styles.addBtnPressed]}>
              <Ionicons name="add" size={18} color={theme.colors.brandMint} />
              <Text style={styles.addBtnText}>Add Another Publication</Text>
            </Pressable>
            ...
            {publications.length > 0 && (
              ...
              >
                <Animated.View style={[styles.saveBtn, { transform: [{ scale: saveScale }] }]}>
                  ...
                  <Text style={styles.saveBtnText}>Save Publications</Text>
                </Animated.View>
              </Pressable>
            )}
